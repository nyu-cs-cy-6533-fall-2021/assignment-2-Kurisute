...
std::vector<VertexBufferObject> VBO(MAX_OBJECT_NUM);
std::vector<VertexBufferObject> VBO_C(MAX_OBJECT_NUM);

// Contains the vertex positions
//Eigen::MatrixXf V(2,3);
std::vector<std::vector<glm::vec2> > V(2);
std::vector<std::vector<glm::vec3> > C(2);
...
int main(void){
    ...
    V[0] = {glm::vec2(0.5, 0),glm::vec2(-0.5, 0),glm::vec2(0, 0.5)};
    C[0] = {glm::vec3(1.0, 0.0, 0.0),glm::vec3(0.0, 1.0, 0.0),glm::vec3(0.0, 0.0, 1.0)};
    ...
    while (!glfwWindowShouldClose(window)){
        ...
        glm::vec2 cursor_pos = cursor_pos_in_window(window);
        V[V.size()-1][V[V.size()-1].size()-1] = cursor_pos;
        ...
        //Draw triangles
        for(int i = 0;i < 1;i++){
            program.bindVertexAttribArray("position",VBO[i]);
            program.bindVertexAttribArray("color",VBO_C[i]);
            VBO[i].update(V[i]);
            VBO_C[i].update(C[i]);
